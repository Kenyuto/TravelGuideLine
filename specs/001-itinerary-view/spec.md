...
使用者可在每個行程卡片中新增購買清單項目，記錄需要購買的物品（名稱、數量、估計金額），並可標記已購買、查看總金額。

**為何此優先級**：提升旅遊規劃完整性，避免遺漏購買項目；支援多人協作記錄購買需求；屬重要但非核心流程。

**獨立測試**：
- 開啟行程卡片 → 查看購買清單區塊 → 新增項目「伴手禮」→ 顯示於清單
- 已新增項目 → 點擊項目切換已購買狀態 → 視覺標記（打勾 + 綠色背景）
- 多個項目 → 顯示總金額與已完成數量（如：「3/5 已完成，總計 NT$ 1,200」）
- 下次載入 → 購買清單狀態保留（LocalStorage 持久化）

**驗收情境**：
1. **Given** 行程卡片展開，**When** 使用者點擊「新增購買項目」，**Then** 顯示輸入表單（項目名稱必填，備註/數量/金額選填）。
2. **Given** 購買清單表單，**When** 使用者輸入項目名稱「伴手禮」並提交，**Then** 項目新增至清單並顯示於卡片下方。
3. **Given** 購買清單項目，**When** 使用者點擊項目或勾選框，**Then** 標記為已購買（視覺：打勾圖示 + 綠色背景）並即時更新統計。
4. **Given** 購買清單包含多個項目，**When** 使用者查看清單，**Then** 顯示已完成數量與總金額（如：「3/5 已完成，總計 NT$ 1,200」）。
5. **Given** 購買清單項目，**When** 使用者點擊刪除按鈕，**Then** 項目從清單中移除並更新統計。
6. **Given** 購買清單有已完成項目，**When** 使用者切換「顯示已完成項目」，**Then** 已完成項目隱藏或顯示。
7. **Given** 已新增購買清單，**When** 使用者重新載入頁面，**Then** 購買清單狀態保留（從 LocalStorage 還原）。
...
- **FR-028**：系統必須在每個行程卡片中提供購買清單功能，使用者可新增、編輯、刪除購買項目。
- **FR-029**：購買清單項目必須包含基本資訊（項目名稱必填）與詳細資訊（備註、數量、估計金額、幣別，皆選填）。
- **FR-030**：系統必須支援購買項目完成狀態切換（`isCompleted`），並於視覺上標示（打勾圖示 + 綠色背景）。
- **FR-031**：系統必須顯示購買清單統計資訊（已完成數量／總數量、總估計金額），並即時更新。
- **FR-032**：系統必須支援過濾已完成項目（顯示全部／僅顯示未完成），使用者可切換顯示模式。
- **FR-033**：購買清單狀態必須儲存於本地（LocalStorage），與行程卡片對應（key: `shopping_${itineraryItemId}`），下次載入時還原。
- **FR-034**：系統應該記錄購買項目的建立者與最後更新者（使用者識別），支援多人協作場景。
- **FR-035**：購買清單項目刪除需要確認（防止誤刪），顯示確認對話框「確定要刪除此項目？」。

...
- **ItineraryItem**：代表單一卡片；屬性：標題、分類、時間段、位置／連結、費用、備註、Emoji、餐廳時段（早餐🌅／午餐☀️／晚餐🌙，僅餐廳類別需填）、Google Maps 連結（可選，建議餐廳必填）。
  - 圖片：`imageUrl`（可選）、`imageAlt`（可選）；若缺失則不顯示圖片區塊。
  - 位置連結：`googleMapsUrl`（可選）；若提供則餐廳名稱顯示為可點擊超連結，開啟 Google Maps（行動裝置使用 app，桌機使用網頁版）。
